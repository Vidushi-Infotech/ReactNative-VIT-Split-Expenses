...
const PaymentHistoryScreen = ({ onClose }) => {
  ...
  const loadPaymentHistory = async () => {
    ...
    try {
      ...
      for (const group of userGroups) {
        try {
          const groupSettlements = await firebaseService.getGroupSettlements(group.id);
          const groupMembers = await firebaseService.getGroupMembersWithProfiles(group.id);
          
          console.log('💳 Group', group.name, '- found', groupSettlements.length, 'settlements');
          ...
          groupSettlements.forEach(settlement => {
            ...
            if (settlement.fromUserId === user.uid || settlement.toUserId === user.uid) {
          ...
          console.error('💳 Error loading settlements for group:', group.id, groupError);
        }
      }
  ...
  const shareIndividualPayment = async (payment) => {
    try {
      const otherUser = payment.isPaid ? payment.toUser.name : payment.fromUser.name;
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.summaryContainer}>
        <View style={styles.summaryCard}>
          ...
          <Text style={[styles.summaryAmount, { color: '#F59E0B' }]}>₹{summary.totalPaid.toFixed(0)}</Text>
        </View>
        <View style={styles.summaryCard}>
          <Text style={styles.summaryLabel}>Total Received</Text>
          <Text style={[styles.summaryAmount, { color: '#10B981' }]}>₹{summary.totalReceived.toFixed(0)}</Text>
        </View>
        <View style={styles.summaryCard}>
      ...
      <View style={styles.filterContainer}>
        ...
        <TouchableOpacity
          style={[styles.filterTab, filterType === 'paid' && styles.activeFilterTab]}
          onPress={() => setFilterType('paid')}
        >
          <Text style={[styles.filterText, filterType === 'paid' && styles.activeFilterText]}>
        ...
        >
          <Text style={[styles.filterText, filterType === 'received' && styles.activeFilterText]}>
      ...
      >
        {loading ? (
          ...
          <View style={styles.emptyContainer}>
            <MaterialIcons name="payment" size={64} color={theme.colors.textMuted} />
          ...
          filteredPayments.map((payment) => (
            <View key={payment.id} style={styles.paymentCard}>
              <View style={styles.paymentHeader}>
                ...
                <View style={styles.paymentInfo}>
                  <Text style={styles.paymentTitle}>
                    ...
                    {payment.isPaid ? payment.toUser.name : payment.fromUser.name}
                  </Text>
              ...
              {payment.description && payment.description !== 'Payment settlement' && (
